...
1、安全性，http是明文传输，数据在传输的过程中不加密，容易篡改；https使用ssl/tls（对称加密和非对称加密结合，非对称加密传递密钥，对称加密用来加密内容）等安全协议来加密传输

2、端口号，80，443

3、证书认证，https使用数字证书来验证服务器身份

4、性能，由于加密和解密数据需要消耗额外的计算资源，https比http稍微慢一些
...
冒泡是事件目标开始冒泡，直到根节点，而捕获则是从更节电开始向下捕获，直到时间目标
...
当一组元素要添加相同的事件时，可以在父元素上绑定一个事件，利用事件冒泡原理，达到父元素代理子元素事件，点击子元素，通过e.target || e.srcElement 可以获取点击的具体子元素
